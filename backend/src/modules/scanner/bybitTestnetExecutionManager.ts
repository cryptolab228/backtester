...
const TESTNET_LOG = logger.child({ module: 'BybitTestnetExecution' });
...
export class BybitTestnetExecutionManager {
  ...
  constructor(executionManager: ExecutionManager) {
    ...
    const creds = config.bybitTestnet;
    if (!(creds?.apiKey && creds?.apiSecret)) {
      TESTNET_LOG.warn('Bybit testnet credentials missing; execution manager will stay idle');
      return;
    }
  ...
  async start(): Promise<void> {
    ...
    TESTNET_LOG.info('Starting initial position sync from Bybit Testnet...');
  ...
  private async syncPositionsFromExchange(): Promise<void> {
    try {
      ...
      const localTestnetPositions = localPositions.filter(p => p.mode === 'testnet');
      ...
      for (const localPos of localTestnetPositions) {
        if (!exchangeSymbols.has(localPos.pairSymbol)) {
          TESTNET_LOG.warn('Closing phantom position - not found on exchange', {
      ...
      if (closedPhantomCount > 0) {
        TESTNET_LOG.info('Closed phantom positions', { count: closedPhantomCount });
      }
      
      if (!positions || positions.length === 0) {
        TESTNET_LOG.info('No open positions on exchange');
        return;
      }
      
      TESTNET_LOG.info('Found positions on exchange to sync', { count: positions.length });
      ...
      TESTNET_LOG.info('Position sync completed', { count: positions.length });
      
      // ✅ Broadcast обновленного snapshot
      await this.executionManager.broadcastSnapshot();
    } catch (error: any) {
      TESTNET_LOG.error('Failed to sync positions from exchange', { 
    ...
    for (const item of payload) {
      ...
      if (positionId) {
        await this.executionManager.attachExternalOrder(positionId, details).catch((error) => {
          TESTNET_LOG.error('Failed to attach external order', { error: error?.message || error, orderId: item.orderId, positionId });
        });
        continue;
      }
  ...
  private async handleExecution(payload: ExecutionReport[]): Promise<void> {
    for (const exec of payload) {
      ...
      if (positionId) {
        await this.executionManager.attachExternalOrder(positionId, details).catch((error) => {
          TESTNET_LOG.error('Failed to attach execution', { error: error?.message || error, orderId: exec.orderId, positionId });
        });
        continue;
      }
  ...
  private async handlePosition(payload: PositionUpdate[]): Promise<void> {
    TESTNET_LOG.debug('Received position update from WebSocket', { count: payload.length });
    ...
    for (const position of payload) {
      ...
      TESTNET_LOG.debug('Processing position update', {
  ...
  private async attachBySymbol(symbol: string, details: ExternalExecutionDetails): Promise<void> {
    ...
    TESTNET_LOG.debug('Searching for position to attach external data', {
      symbol,
    ...
    let match = openPositions.find((pos) => pos.pairSymbol === symbol && pos.mode === 'testnet');
    ...
    if (!match && details.filledQty && Number(details.filledQty) > 0) {
      TESTNET_LOG.warn('Exchange position not tracked locally - creating record', { 
        symbol, 
      ...
      if (sessionId) {
        try {
          ...
          if (entryPrice === 0) {
            TESTNET_LOG.error('Cannot save position without price', { symbol, avgPrice: details.avgPrice, markPrice: details.markPrice });
            return;
          }
          
          TESTNET_LOG.debug('Preparing to save exchange position', {
            symbol,
            filledQty,
            direction,
            size,
          ...
          TESTNET_LOG.info('Exchange position saved to DB', { 
            symbol, 
            sessionId,
          ...
          if (typeof (this.executionManager as any).restorePosition === 'function') {
            ...
            if (match) {
              await this.executionManager.updatePositionRecord(match.id, { mode: 'testnet' });
              ...
              TESTNET_LOG.info('Restored position from previous session with PnL data', { 
                positionId: match.id, 
                symbol,
                unrealizedPnl: details.unrealizedPnl,
                markPrice: details.markPrice,
          ...
          TESTNET_LOG.error('Failed to save/restore exchange position', { 
            symbol, 
            error: error?.message || error 
          });
        }
    ...
    if (!match) {
      TESTNET_LOG.warn('No matching testnet position for symbol and failed to create', { symbol, details });
      return;
    }

    TESTNET_LOG.debug('Attaching external data to position', {
      positionId: match.id,
      symbol,
      unrealizedPnl: details.unrealizedPnl,
      markPrice: details.markPrice,
    ...
    await this.executionManager.attachExternalOrder(match.id, {
      ...
      TESTNET_LOG.error('Failed to attach aggregated update', { error: error?.message || error, symbol, positionId: match.id });
    });
  }

  /**
   * Получение позиций с биржи Bybit Testnet
   */
  async getExchangePositions(symbol?: string): Promise<any[]> {
    if (!this.tradingClient) {
      TESTNET_LOG.warn('Trading client not initialized');
      return [];
    }
    ...
    try {
      ...
      TESTNET_LOG.debug('Fetched positions from Bybit Testnet', {
      ...
      TESTNET_LOG.error('Failed to fetch positions from Bybit Testnet', {
        error: error?.message || error,
        symbol,
      });
      return [];
  ...
  /**
   * Закрытие позиции на бирже Bybit Testnet
   */
  async closeExchangePosition(symbol: string, positionIdx: number = 0): Promise<boolean> {
    if (!this.tradingClient) {
      TESTNET_LOG.warn('Trading client not initialized, cannot close position');
      return false;
    }
    ...
    try {
      ...
      TESTNET_LOG.info('Position closed on Bybit Testnet', {
        symbol,
      ...
      TESTNET_LOG.error('Failed to close position on Bybit Testnet', {
        error: error?.message || error,
        symbol,
        positionIdx,
      });
  ...
  /**
   * Отмена всех открытых ордеров на бирже Bybit Testnet (опционально по символу)
   */
  async cancelAllOrders(symbol?: string): Promise<boolean> {
    if (!this.tradingClient) {
      TESTNET_LOG.warn('Trading client not initialized, cannot cancel orders');
      return false;
    }
    ...
    try {
      ...
      TESTNET_LOG.error('Failed to cancel all orders on Bybit Testnet', {
        error: error?.message || error,
        symbol,
      });
      return false;
  ...
  async getOrderHistory(params?: { symbol?: string; limit?: number; startTime?: number; endTime?: number }): Promise<any[]> {
    ...
    try {
      ...
      TESTNET_LOG.debug('Fetched order history from Bybit Testnet', {
        count: orders.length,
      ...
      TESTNET_LOG.error('Failed to fetch order history from Bybit Testnet', {
        error: error?.message || error,
      });
      return [];
    }
  ...
  async getTradeHistory(params?: { symbol?: string; limit?: number; startTime?: number; endTime?: number }): Promise<any[]> {
    ...
    try {
      ...
      TESTNET_LOG.debug('Fetched trade history from Bybit Testnet', {
        count: trades.length,
        symbol: params?.symbol,
      });
      ...
      TESTNET_LOG.error('Failed to fetch trade history from Bybit Testnet', {
        error: error?.message || error,
      });
      return [];
    }
  ...
  async getClosedPnL(params?: { symbol?: string; limit?: number; startTime?: number; endTime?: number }): Promise<any[]> {
    ...
    try {
      ...
      TESTNET_LOG.debug('Fetched closed PnL from Bybit Testnet', {
        count: pnl.length,
        symbol: params?.symbol,
      });
      ...
      TESTNET_LOG.error('Failed to fetch closed PnL from Bybit Testnet', {
        error: error?.message || error,
      });
      return [];
    }
  ...
  async calculateTotalPnL(symbol?: string): Promise<any> {
    ...
    try {
      ...
      TESTNET_LOG.info('Calculated total PnL for Bybit Testnet', {
        symbol: symbol || 'all',
      ...
      TESTNET_LOG.error('Failed to calculate total PnL for Bybit Testnet', {
        error: error?.message || error,
      });
      return {
        realizedPnL: 0,
