...
class DataController {
  ...
  async forceKillJob(req: Request, res: Response): Promise<void> {
    ...
    try {
      const job = await dataQueue.getJob(jobId);
      if (!job) {
        logger.warn(`[ForceKill] Job ${jobId} not found.`);
        res.status(404).json({ message: 'Job not found' });
        return;
      }
      ...
      logger.info(`[ForceKill] Attempting to force kill job ${jobId} in state: ${state}`);

      if (state === 'active') {
        // Для активных задач - отправляем "сигнал" на остановку через Redis
        const killKey = `job:${jobId}:kill`;
        await redis.set(killKey, '1', 'EX', 3600); // Сигнал живет 1 час
        logger.info(`[ForceKill] Kill signal sent to active job ${jobId} via Redis key ${killKey}`);
      } else if (['waiting', 'delayed', 'paused'].includes(state)) {
        // Для неактивных задач - просто удаляем
        await job.remove();
        logger.info(`[ForceKill] Successfully removed job ${jobId} from queue (state: ${state})`);
      } else {
        // Для завершенных/failed задач - просто удаляем
        await job.remove();
        logger.info(`[ForceKill] Removed completed/failed job ${jobId} (state: ${state})`);
      }

      res.status(200).json({ 
        message: `Job ${jobId} force killed successfully`,
        previousState: state
      });
    } catch (error: any) {
      logger.error(`[ForceKill] Error force killing job ${jobId}:`, error);
      res.status(500).json({ 
  ...
  private async cleanupJobResources(jobId: string): Promise<void> {
    logger.debug(`[Cleanup] Cleaning up resources for job ${jobId}`);
    
    // Принудительный garbage collection для освобождения памяти
    if (global.gc) {
      global.gc();
      logger.debug(`[Cleanup] Triggered garbage collection for job ${jobId}`);
    } else {
      logger.warn(`[Cleanup] Garbage collection not available (--expose-gc flag needed)`);
    }
