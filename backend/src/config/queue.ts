...
// Функция для создания инстанса воркера с высокопроизводительными настройками
export const createWorker = <T = any, R = any>(queueName: string, processor: (job: Job<T>) => Promise<R>) => {
  return new Worker<T, R>(queueName, processor, {
    connection,
    concurrency: 3, // 3 параллельных воркера для 3 CPU cores
    maxStalledCount: 5, // Увеличенное количество попыток при зависании
    stalledInterval: 60000, // 60 секунд для больших задач
    // Управление памятью реализовано в dataWorker.ts через мониторинг и GC
  });
};
