...
// Базовый URL бэкенда (нормализуем до .../api)
const RAW_API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000';
const NORMALIZED_BASE = RAW_API_BASE_URL.endsWith('/api')
  ? RAW_API_BASE_URL
...
export const forceKillJob = async (jobId: string): Promise<{ message: string; previousState: string }> => {
  const response = await apiClient.post(`/data/queue/jobs/${jobId}/force-kill`);
  return response.data;
};
...
// НОВОЕ: Функции для работы с GPU сервисом
...
export const checkGPUHealth = async (): Promise<GPUHealthResponse> => {
  try {
    const response = await apiClient.get<GPUHealthResponse>('/backtest/gpu/health');
    return response.data;
  } catch (error: any) {
    console.error('[ApiService] Error checking GPU health:', error.response?.data || error.message);
    return {
      success: false,
      gpu_available: false,
