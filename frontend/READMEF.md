...
*   **`settingsStore.ts`**: Для управления параметрами самой стратегии (DLC, NWE, кластеры, риски и т.д.), которые загружаются из localStorage и могут быть изменены пользователем в `StrategySettingsForm.vue`.
